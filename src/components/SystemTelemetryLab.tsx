...
const NODES: ArchNode[] = [
  {
    ...
    label: "CLIENT",
    icon: Terminal,
    purpose: "Initiates request and consumes UI state.",
    x: 0,
    y: 0,
  },
  {
    ...
    label: "API GATEWAY",
    icon: Webhook,
    purpose: "Ingresses traffic, terminates SSL, rate-limits.",
    x: 1,
    y: 0,
  },
  {
    ...
    label: "VALIDATION",
    icon: ShieldAlert,
    purpose: "Sanitizes payloads and verifies auth JWTs.",
    x: 2,
    y: 0,
  },
  {
  ...
  },
  {
    ...
    label: "DATABASE",
    icon: Database,
    purpose: "ACID transactions & relational persistence.",
    x: 4,
    y: 0,
  },
  {
    ...
    label: "EVENT BUS",
    icon: Rss,
    purpose: "Decouples async state mutations (Pub/Sub).",
    x: 4,
    y: 1,
  },
  {
    ...
    label: "WEBSOCKET",
    icon: Server,
    purpose: "Maintains full-duplex client connections.",
    x: 2,
    y: 1,
  },
  {
    ...
    label: "CLIENT POOL",
    icon: Users,
    purpose: "Multiple consumers receive live broadcast.",
    x: 0,
    y: 1,
  },
];
...
const FLOWS: Record<SimMode, SimStep[]> = {
  ...
  TRANSACTION: [
    ...
    { nodeId: "db", status: "success", log: "COMMIT_SUCCESS", durationMs: 500 },
    {
      nodeId: "service",
      status: "success",
      log: "TRANSACTION_CLOSED",
      durationMs: 300,
    },
    {
      nodeId: "client",
      status: "success",
      log: "UI_OPTIMISTIC_CONFIRM",
      durationMs: 600,
    },
  ],
  "REAL-TIME": [
    ...
    },
    {
      nodeId: "socket",
      status: "running",
      log: "BROADCAST_FANOUT",
      durationMs: 400,
    },
    {
      nodeId: "client_sync",
      status: "success",
      log: "CLIENTS_SYNCHRONIZED",
      durationMs: 800,
    },
  ],
  FAILURE: [
    ...
    },
    {
      nodeId: "api",
      status: "error",
      log: "503_ERROR_THROWN",
      durationMs: 300,
    },
    {
      nodeId: "client",
      status: "error",
      log: "RECOVERY_STATE_LOADED",
      durationMs: 800,
    },
  ],
};
...
export function SystemTelemetryLab() {
  ...
  const startSimulation = (selectedMode: SimMode = mode) => {
    ...
    if (reducedMotion) {
      setLogs(
        FLOWS[selectedMode].map((step, index) => ({
          time: `STEP ${index + 1}`,
          msg: step.log,
          type: step.status,
        })),
  ...
  const currentStatus =
    activeStep >= 0 ? currentFlow[activeStep].status : "idle";

  return (
    <div
      ref={labRef}
      className="w-full bg-[var(--bg-secondary)] font-mono text-[var(--text-primary)] border border-[var(--surface-border)] shadow-[var(--card-shadow)]"
    >
      {/* Header Panel */}
      <div className="flex flex-col sm:flex-row items-stretch justify-between border-b border-[var(--surface-border)] bg-[var(--bg-tertiary)]">
        <div className="px-4 py-3 flex items-center justify-between sm:justify-start gap-4 border-b sm:border-b-0 border-[var(--surface-border)]">
          ...
          <span className="px-1.5 py-0.5 bg-[var(--bg-primary)] text-[9px] uppercase border border-[var(--surface-border)] text-[var(--text-primary)] font-bold">
            {isRunning ? "ACTIVE" : "READY"}
          </span>
        </div>

        <div className="flex items-center overflow-x-auto no-scrollbar border-l border-[var(--surface-border)]">
          {(Object.keys(FLOWS) as SimMode[]).map((m) => (
            <button
              key={m}
              onClick={() => handleModeSelect(m)}
              className={`px-4 py-3 text-[10px] sm:text-xs font-bold uppercase whitespace-nowrap transition-colors border-r border-[var(--surface-border)] ${
                mode === m && isRunning
                  ? "bg-[var(--accent-gold)] text-[#090a0f] shadow-sm"
      ...
      <div className="relative p-6 sm:p-12 min-h-[440px] flex items-center justify-center overflow-hidden z-0">
        <div
          ...
          style={{
            backgroundImage:
              "linear-gradient(#F4F3EE 1px, transparent 1px), linear-gradient(90deg, #F4F3EE 1px, transparent 1px)",
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 lg:gap-8 relative w-full max-w-5xl z-10">
          {NODES.map((node) => {
            ...
            let colorCls =
              "border-[var(--surface-border)] text-[var(--text-muted)] bg-[var(--bg-secondary)] hover:border-[var(--accent-gold)] hover:text-[var(--text-primary)]";
            ...
            return (
              <motion.button
                ...
                onClick={() =>
                  setInspectedNode(inspectedNode === node.id ? null : node.id)
                }
                onMouseEnter={() => {
                  soundEngine.tick();
                ...
                <AnimatePresence>
                  {inspectedNode === node.id && (
                    <motion.div
                      ...
                      className="absolute bottom-[115%] left-1/2 -translate-x-1/2 w-48 sm:w-56 p-3 bg-[var(--bg-secondary)] border border-[var(--surface-border-strong)] z-50 shadow-[var(--card-shadow)] text-[var(--text-primary)] text-left pointer-events-none"
                    >
                      <strong className="block text-[10px] sm:text-xs mb-1.5 font-bold text-[var(--accent-gold)] bg-[var(--bg-tertiary)] border border-[var(--surface-border)] px-1.5 py-0.5 inline-block">
                        {node.label}
                      </strong>
                      <p className="text-[10px] sm:text-xs m-0 leading-snug font-sans font-medium text-[var(--text-secondary)]">
                        {node.purpose}
                      </p>
                    </motion.div>
                  )}
      ...
      {/* Telemetry Footer */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-0 border-t border-[var(--surface-border)] bg-[var(--bg-tertiary)]">
        {/* State Panel */}
        <div className="md:col-span-4 p-4 sm:p-5 border-b md:border-b-0 md:border-r border-[var(--surface-border)]">
          <div className="text-[10px] sm:text-xs text-[var(--text-muted)] font-bold uppercase tracking-widest mb-4">
            Core Telemetry
          </div>
            ...
            {currentNodeId ? (
              <>
                <div className="flex justify-between text-[11px] sm:text-xs items-center">
                  <span className="text-[var(--text-secondary)]">NODE_ID</span>
                  <span className="text-[#E3C849] font-bold bg-[#E3C849]/10 px-1.5 py-0.5">
                    {currentNodeId}
                  </span>
                </div>
                <div className="flex justify-between text-[11px] sm:text-xs items-center">
                  ...
                  <span
                    className={
                      currentStatus === "error"
                        ? "text-[#FF5500] font-bold uppercase"
                        : currentStatus === "success"
                          ? "text-[#10B981] font-bold uppercase"
                          : "text-[#38BDF8] font-bold uppercase"
                    }
                  >
                    {currentStatus}
                  </span>
                ...
                <div className="flex justify-between text-[11px] sm:text-xs items-center">
                  ...
                  <span className="text-[var(--text-primary)] tabular-nums">
                    {currentFlow[activeStep].durationMs}ms
                  </span>
                </div>
              </>
            ) : (
              <div className="text-xs text-[var(--text-muted)] italic h-[76px] flex items-center justify-center border border-dashed border-[var(--surface-border)]">
        ...
        {/* Console Log */}
        <div
          className="md:col-span-8 p-4 sm:p-5 bg-[var(--bg-primary)] h-[160px] overflow-y-auto"
          ref={logRef}
        >
          {logs.length === 0 ? (
            <div className="text-xs text-[var(--text-muted)] italic">
              Booting architecture metrics...
            </div>
          ) : (
            logs.map((L, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`text-[10px] sm:text-[11px] mb-2 flex gap-3 ${color} font-mono leading-tight`}
                >
